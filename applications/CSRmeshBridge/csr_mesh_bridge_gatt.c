...
{
    if (g_bridgeapp_data.gatt_data.gatt_advert_tid == tid)
    {
        g_bridgeapp_data.gatt_data.gatt_advert_tid = TIMER_INVALID;
...
extern void GattTriggerConnectableAdverts(void)
{
    ...
    MemCopy(&gatt_adv_data.adv_params.bd_addr.addr, 
            &g_bridgeapp_data.random_bd_addr, 
            sizeof(BD_ADDR_T));
    ...
    g_bridgeapp_data.gatt_data.advert_timer_value = ADVERT_INTERVAL;

    /* Restart the advertising timer */
    TimerDelete(g_bridgeapp_data.gatt_data.gatt_advert_tid);
    g_bridgeapp_data.gatt_data.gatt_advert_tid = TimerCreate(
