...
CSRMESH_SWITCH_APP_DATA_T g_switchapp_data;
/* Declare space for Model Groups */
uint16 switch_model_groups[MAX_MODEL_GROUPS];
...
uint16 wdog_model_groups[MAX_MODEL_GROUPS];
#endif
...
{
    g_switchapp_data.le_params.generic_le_param.scan_param.scan_duty_param.scan_duty_cycle 
                                                              = scan_duty_cycle;
    g_switchapp_data.le_params.generic_le_param.scan_param.scan_duty_param.min_scan_slot = 4;
    g_switchapp_data.le_params.generic_le_param.scan_param_type = CSR_SCHED_SCAN_DUTY_PARAM;
    CSRSchedSetConfigParams(&g_switchapp_data.le_params);
}
 ...
 *
 *  DESCRIPTION
 *      This function is called to initialise CSRmesh switch application
 *      data structure.
 *
 *  RETURNS
 *      Nothing.
...
void AppInit(sleep_state last_sleep_state)
{
    ...
    */
    ...
    generateStaticRandomAddress(&g_switchapp_data.random_bd_addr);

    /* Set the Static Random Address of the device. */
    GapSetRandomAddress(&g_switchapp_data.random_bd_addr);
    ...
    SetLeConfigParams(&g_switchapp_data.le_params);
    CSRSchedSetConfigParams(&g_switchapp_data.le_params);
    ...
    if(result == CSR_MESH_RESULT_SUCCESS)
    {
        ...
        LightModelClientInit(AppLightClientHandler);

        /* Initialize the power Client */
        PowerModelClientInit(AppPowerClientHandler);
        ...
        AppWatchdogModelInit(wdog_model_groups, MAX_MODEL_GROUPS);
        ...
        if(result == CSR_MESH_RESULT_SUCCESS)
        {
             ...
             */
            g_switchapp_data.bearer_tx_state.mcpRelayInfo.relayEnable = TRUE;
            g_switchapp_data.bearer_tx_state.mcpRelayInfo.netId = 
                                                        CSR_MESH_DEFAULT_NETID;
            g_switchapp_data.bearer_tx_state.promiscuousEnable = TRUE;
            g_switchapp_data.bearer_tx_state.maspRelayEnable = TRUE;

            /* Post MASP_DEVICE_ID msg in Bearer TX queue */
            if(g_switchapp_data.assoc_state == app_state_not_associated)
            {
                DEBUG_STR("Switch not associated\r\n");
    ...
    if (old_state != new_state)
    {
        ...
        switch (old_state)
        {
            case app_state_disconnecting:
                ...
                /* Initialise CSRmesh switch data and services
