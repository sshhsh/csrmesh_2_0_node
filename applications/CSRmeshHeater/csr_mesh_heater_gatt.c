 ...
 *
 *  DESCRIPTION
 *      Implementation of the GATT-related routines of the CSRmesh Heater
 *
 *****************************************************************************/
...
{
    if (g_heater_app_data.gatt_data.gatt_advert_tid == tid)
    {
        g_heater_app_data.gatt_data.gatt_advert_tid = TIMER_INVALID;
...
extern void GattTriggerConnectableAdverts(void)
{
    ...
    if(!(g_heater_app_data.bearer_tx_state.bearerEnabled &
    ...
    MemCopy(&gatt_adv_data.adv_params.bd_addr.addr,
            &g_heater_app_data.random_bd_addr, 
            sizeof(BD_ADDR_T));
    ...
    g_heater_app_data.gatt_data.advert_timer_value = ADVERT_INTERVAL;

    /* Restart the advertising timer */
    TimerDelete(g_heater_app_data.gatt_data.gatt_advert_tid);
    g_heater_app_data.gatt_data.gatt_advert_tid = TimerCreate(
                               g_heater_app_data.gatt_data.advert_timer_value,
