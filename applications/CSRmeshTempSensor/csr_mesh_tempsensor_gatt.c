 ...
 *
 *  DESCRIPTION
 *      Implementation of the CSRmesh Temperature Sensor GATT-related routines
 *
 *****************************************************************************/
 ...
 *  Private Data
 *===========================================================================*/
static CSR_SCHED_ADV_DATA_T gatt_adv_data;
...
static bool csrStoreUserAdvData(uint16 len, uint8 *data, ad_src const src)
{
    ...
    {
        p_data = gatt_adv_data.ad_data;
    }
    else
    {
        p_data = gatt_adv_data.scan_rsp_data;
    }
...
{
    if (g_tsapp_data.gatt_data.gatt_advert_tid == tid)
    {
        g_tsapp_data.gatt_data.gatt_advert_tid = TIMER_INVALID;
...
extern void GattTriggerConnectableAdverts(void)
{
    ...
    if(!(g_tsapp_data.bearer_tx_state.bearerEnabled & GATT_SERVER_BEARER_ACTIVE))
    {
    ...
    MemCopy(&gatt_adv_data.adv_params.bd_addr.addr, &g_tsapp_data.random_bd_addr, 
                                                            sizeof(BD_ADDR_T));
    ...
    gatt_adv_data.ad_data_length = MAX_ADV_DATA_LEN;
    gatt_adv_data.scan_rsp_data_length = MAX_ADV_DATA_LEN;
    ...
    TimerDelete(g_tsapp_data.gatt_data.gatt_advert_tid);
    g_tsapp_data.gatt_data.gatt_advert_tid = TimerCreate(ADVERT_INTERVAL,
