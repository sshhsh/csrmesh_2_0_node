...
extern void ReadPersistentStore(void)
{
    ...
    if(nvm_sanity == NVM_SANITY_MAGIC)
    {
        ...
        Nvm_Read((uint16 *)&g_tsapp_data.bearer_tx_state, 
          ...
          */
    {
        ...
        g_tsapp_data.bearer_tx_state.bearerEnabled =  LE_BEARER_ACTIVE;
        ...
        Nvm_Write((uint16 *)sensor_model_groups, 
                  sizeof(sensor_model_groups),
                  NVM_OFFSET_SENSOR_MODEL_GROUPS);
...
extern void InitiateAssociation(void)
{
     ...
     */
    g_tsapp_data.mesh_device_id_advert_tid =
...
extern bool HandleGroupSetMsg(CSR_MESH_GROUP_ID_RELATED_DATA_T msg)
{
    ...
    if(model == CSRMESH_SENSOR_MODEL || model == CSRMESH_ACTUATOR_MODEL ||
       model == CSRMESH_ALL_MODELS)
    {
        if(index < NUM_SENSOR_MODEL_GROUPS)
        {
            ...
            Nvm_Write(&sensor_model_groups[index], 
             ...
             ConfigureSensor(old_config_status);
