...
CSRMESH_TEMP_SENSOR_APP_DATA_T g_tsapp_data;
...
/* sensor model groups are shared with actuator model in this application */
...
{
    g_tsapp_data.le_params.generic_le_param.scan_param.scan_duty_param.scan_duty_cycle 
                                                              = scan_duty_cycle;
    g_tsapp_data.le_params.generic_le_param.scan_param.scan_duty_param.min_scan_slot = 4;
    g_tsapp_data.le_params.generic_le_param.scan_param_type = CSR_SCHED_SCAN_DUTY_PARAM;
    CSRSchedSetConfigParams(&g_tsapp_data.le_params);
}
...
void AppInit(sleep_state last_sleep_state)
{
    ...
    */
    ...
    SetLeConfigParams(&g_tsapp_data.le_params);
    CSRSchedSetConfigParams(&g_tsapp_data.le_params);
    ...
    if(result == CSR_MESH_RESULT_SUCCESS)
    {
        ...
        ActuatorModelInit(0, sensor_model_groups, NUM_SENSOR_MODEL_GROUPS,
        ...
        if(result == CSR_MESH_RESULT_SUCCESS)
        {
             ...
             */
            g_tsapp_data.bearer_tx_state.mcpRelayInfo.relayEnable = TRUE;
            g_tsapp_data.bearer_tx_state.mcpRelayInfo.netId = 
                                                        CSR_MESH_DEFAULT_NETID;
            g_tsapp_data.bearer_tx_state.promiscuousEnable = TRUE;
            g_tsapp_data.bearer_tx_state.maspRelayEnable = TRUE;

            if(g_tsapp_data.assoc_state == app_state_not_associated)
            {
                DEBUG_STR("Temp Sensor not associated\r\n");
            ...
            {
                /* Device already associated. start reading the temp if the 
                 * sensor is already configured.
                 */
                EnableTempReading();
            }
...
extern void AppProcessSystemEvent(sys_event_id id, void *data)
{
    switch (id)
    {
        ...
        {
#if defined(ENABLE_TEMPERATURE_CONTROLLER) && !defined(DEBUG_ENABLE)
             HandlePIOEvent(((pio_changed_data*)data)->pio_cause);
#endif /* defined(ENABLE_TEMPERATURE_CONTROLLER) && !defined(DEBUG_ENABLE) */
        }
        break;
