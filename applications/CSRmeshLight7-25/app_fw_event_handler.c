                                     ...
                                     LM_EV_CONNECTION_COMPLETE_T *p_event_data)
{
    ...
    g_lightapp_data.gatt_event_data.conn_interval = 
                                        p_event_data->data.conn_interval;
}
    ...
    switch(g_lightapp_data.state)
    {
        case app_state_advertising:
        {
            if(p_event_data->result == sys_status_success)
            {
                ...
                g_lightapp_data.gatt_event_data.conn_interval = 
                                                            p_event_data->cid;
                g_lightapp_data.gatt_event_data.is_gatt_bearer_ready = TRUE;
                 ...
                 */
                LsRadioEventNotification(g_lightapp_data.gatt_data.st_ucid, 
...
extern bool HandleLEAdvMessage(LM_EV_ADVERTISING_REPORT_T* report)
{
    ...
    if( data->event_type == ls_advert_non_connectable )
    {
        ...
        do
        {
            if(index + 4 < data->length_data)
            {
                ...
                else if (!MemCmp(&unpackedData[index + 1], mydata, 3))
                {
                    /* Not a mesh data */
					/*DEBUG_STR(" a phone address data ");*/
					
					
					CSRMESH_DATA_BLOCK_SEND_T p_send;
					BD_ADDR_T *myaddress=&(data->address);
					
					p_send.datagramoctets[0]=USER_LOCATION_REPORT;
					p_send.datagramoctets[1]=(myaddress->nap & 0xFF00)>>8;
					p_send.datagramoctets[2]=(myaddress->nap & 0x00FF);
					p_send.datagramoctets[3]=myaddress->uap;
					p_send.datagramoctets[4]=(myaddress->lap & 0xFF0000) >> 16;
					p_send.datagramoctets[5]=(myaddress->lap & 0x00FF00) >> 8;
					p_send.datagramoctets[6]=(myaddress->lap & 0x0000FF);
                    p_send.datagramoctets[7]=(CsrUint8)report->rssi;
                    

					p_send.datagramoctets_len=10;
					
					/*DEBUG_U8((myaddress->nap & 0xFF00)>>8);*/
					/*DEBUG_STR(":");*/
    				/*DEBUG_U8((myaddress->nap & 0x00FF));*/
					/*DEBUG_STR(":");*/
    				/*DEBUG_U8(myaddress->uap);*/
					/*DEBUG_STR(":");*/
    				/*DEBUG_U8((myaddress->lap & 0xFF0000) >> 16);*/
					/*DEBUG_STR(":");*/
    				/*DEBUG_U8((myaddress->lap & 0x00FF00) >> 8);*/
					/*DEBUG_STR(":");*/
    				/*DEBUG_U8((myaddress->lap & 0x0000FF));*/
					/*DEBUG_STR("\n")*/
                    
					/*DEBUG_U8(report->rssi);*/
                    

					
					DataBlockSend(g_lightapp_data.netId,0x8ffe,&p_send);
    ...
    else
    {
          ...
          */
          /*DEBUG_STR("connectable undirected adv");*/
    }
